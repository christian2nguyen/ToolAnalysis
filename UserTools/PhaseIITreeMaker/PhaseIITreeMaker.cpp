...
void PhaseIITreeMaker::FillSimpleRecoInfo() {
  ...
  if(get_flag && get_energy && get_vtx && 
  get_stopvtx && get_cos && get_pt &&
  get_fv && get_mrdenergy && get_mrdtrack &&
  get_mrdstart && get_mrdstop){
...
//    }
  ...
  if(verbosity>1)std::cout <<"MCNeutCapGammas count CaptGammas: "<<MCNeutCapGammas.count("CaptGammas")<<std::endl;
  if (MCNeutCapGammas.count("CaptGammas")>0){
    ...
    for (int i_cap = 0; i_cap < (int) cap_energies.size(); i_cap++){
      for (int i_gamma=0; i_gamma < cap_energies.at(i_cap).size(); i_gamma++){
        if(verbosity>2) std::cout <<"gamma energy: "<<cap_energies.at(i_cap).at(i_gamma)<<std::endl;
  ...
  if (hasGenie){
    ...
    bool get_fsl_energy = m_data->Stores["GenieInfo"]->Get("FSLeptonEnergy",TrueFSLeptonEnergy); // FSLeptonEnergy is in MeV assuming its total energy 
    ...
    if(verbosity>1)std::cout <<"get_neutrino_energy: "<<get_neutrino_energy<<"get_neutrino_vtxx: "<<get_neutrino_vtxx<<"get_neutrino_vtxy: "<<get_neutrino_vtxy<<"get_neutrino_vtxz: "<<get_neutrino_vtxz<<"get_neutrino_time: "<<get_neutrino_vtxt<<std::endl;
    if(verbosity>1)std::cout <<"get_q2: "<<get_q2<<", get_cc: "<<get_cc<<", get_qel: "<<get_qel<<", get_res: "<<get_res<<", get_dis: "<<get_dis<<", get_coh: "<<get_coh<<", get_mec: "<<get_mec<<std::endl;
    if(verbosity>1)std::cout <<"get_n: "<<get_n<<", get_p: "<<get_p<<", get_pi0: "<<get_pi0<<", get_piplus: "<<get_piplus<<", get_pipluscher: "<<get_pipluscher<<", get_piminus: "<<get_piminus<<", get_piminuscher: "<<get_piminuscher<<", get_kplus: "<<get_kplus<<", get_kpluscher: "<<get_kpluscher<<", get_kminus: "<<get_kminus<<", get_kminuscher: "<<get_kminuscher<<std::endl;
    if(verbosity>1)std::cout <<"get_fsl_vtx: "<<get_fsl_vtx<<", get_fsl_momentum: "<<get_fsl_momentum<<", get_fsl_time: "<<get_fsl_time<<", get_fsl_mass: "<<get_fsl_mass<<", get_fsl_pdg: "<<get_fsl_pdg<<", get_fsl_energy: "<<get_fsl_energy<<std::endl;
    if(verbosity>1)std::cout <<"get_bjx: "<<get_bjx<<", get_y: "<<get_y<<", get_targetZ: "<<get_targetZ<<", get_q0: "<<get_q0<<", get_q3: "<<get_q3<< "get_nu_pdg = "<< get_nu_pdg<<std::endl;
    if (get_neutrino_energy && get_neutrino_mom && get_neutrino_vtxx && 
    get_neutrino_vtxy && get_neutrino_vtxz && get_neutrino_vtxt && 
    get_q2 && get_cc && get_nc && get_qel && get_res && get_dis &&
    get_coh && get_mec && get_n && get_p && get_pi0 && get_piplus && 
    get_pipluscher && get_piminus && get_piminuscher && get_kplus && 
    get_kpluscher && get_kminus && get_kminuscher && get_fsl_vtx &&
    get_fsl_momentum && get_fsl_time && get_fsl_mass && get_fsl_pdg && 
    get_fsl_energy && get_bjx && get_y && get_targetZ && get_q0 && get_q3 && get_w ){
      ...
      if(CC0PiSignal_fill){
          ...
          bool insideFV = FidVol(TrueNuIntxVtx_X, TrueNuIntxVtx_Y- DisplacementTRUE_Y, TrueNuIntxVtx_Z+ DisplacementTRUE_Z);
          FillCC0pi_categorize_event(hasGenie, insideFV, TrueCC,   TrueNuPDG,
                                   Event_PDGs, Event_P, TrueFSLeptonPdg, KE_FS_muon);  
          }
         ...
         if(get_vtxzDistanceToEdge && VertexDistancetoEdge_fill ){
            fTrueDistanceToEdge = TrueNuIntxVtxDisToEdge;  
      }
...
void PhaseIITreeMaker::FillCC0pi_categorize_event(bool IsMC, bool insideFV, int isCC, int TrueNuPDG,
                                   std::vector<int> Event_PDGs, std::vector<double> Event_P, 
                                   int TrueFSLPdg,
                                   double inputMC_Pmu) {
  ...
  if ( !fTruemc_vertex_in_FV ) {
   ...
   fTruemc_is_cc0pi_signal = 0;
    if ( TrueNuPDG == ELECTRON_NEUTRINO
      && isCC==0 ) {fTrue_category= kNuECC; return;}
    else {
...
if(verbosity>=2) std::cout<<"inputMC_Pmu = "<< inputMC_Pmu << " TrueFSLPdg = "<< TrueFSLPdg<< std::endl;
  ...
  if ( inputMC_Pmu >= MUON_P_MIN_MOM_CUT && inputMC_Pmu <= MUON_P_MAX_MOM_CUT  && TrueFSLPdg==MUON) {
 ...
 for(size_t i = 0; i < Event_PDGs.size(); ++i){
  ...
  fTruemc_is_cc0pi_signal = (fTruemc_vertex_in_FV==1 &&
                             fTruemc_neutrino_is_numu==1 && 
                             fTruemc_muon_in_mom_range==1 &&
                             fTruemc_no_fs_mesons ==1 && 
                             fTruemc_no_charged_pi_above_threshold==1 &&
                             fTruemc_no_fs_pi0 ==1) ? 1 : 0;
  ...
  if ( fTruemc_is_cc0pi_signal ) {
    ...
    else {fTrue_category= kSignalOther; 
          return; }
  }
  ...
  else {fTrue_category= kNuMuCCOther;
       return;}
}
